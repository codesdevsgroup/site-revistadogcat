...
export class ArtigoLeituraComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.artigosService.obterArtigo(id).subscribe({
      next: (art) => {
        this.artigo = art;
        this.conteudoSeguro = this.sanitizer.bypassSecurityTrustHtml(this.extractHtmlFromContent(art.conteudo));
        this.loading = false;
      },
      error: (err) => {
        console.error('Erro ao carregar artigo:', err);
        this.error = 'Não foi possível carregar o artigo. Tente novamente mais tarde.';
        this.loading = false;
      }
    });
  }
      ...
      if (typeof content === 'object') {
        ...
        if ('content' in content && Array.isArray(content.content)) {
          const parts = content.content
            .map((node: any) => node?.text || node?.content?.map((n: any) => n.text).join(' ') || '')
            .filter(Boolean);
